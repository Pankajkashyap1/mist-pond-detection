#!/bin/bash
# ==============================================================================
# Robust 4-Node Cluster Deployment & Tunneling Script
# Replaces fuser with pkill, installs dependencies, and ensures remote web_app.py stability.
# ==============================================================================
...
for ITEM in "${NODES[@]}"; do
    ...
    # Step B: Unpack, kill old worker safely, install dependencies, and start worker process
    ssh -o StrictHostKeyChecking=no -p $SSH_PORT student@10.1.75.51 "mkdir -p ~/smart_pond_detection && tar -xzf ~/smart_pond_bundle.tar.gz -C ~/smart_pond_detection/ && cd ~/smart_pond_detection && (pkill -9 -f 'python3 web_app.py' 2>/dev/null || true) && (python3 -m pip install --user flask numpy requests 2>/dev/null || true) && PORT=5000 nohup python3 web_app.py > worker_5000.log 2>&1 &"
